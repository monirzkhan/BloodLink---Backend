...
const createBloodRequest = catchAsync(async (req: Request, res: Response) => {
	...
	sendResponse(res, {
		...
		success: true,
		message:
			"Blood request created successfully and is waiting for verification",
		data: result,
	});
});
const verifyBloodRequestByAdmin = catchAsync(
	async (req: Request, res: Response) => {
		...
		const result = await bloodRequestService.verifyBloodRequestByAdmin(
			userId,
			id as string,
			payload,
		);
		...
		sendResponse(res, {
			...
			success: true,
			message:
				"Blood request Verified successfully and is waiting for Donor Match",
			data: result,
		});
	},
);
