...
import type { Candidate } from "./bloodRequest.interface";
...
export const findAndMatchDonors = async (requestId: string) => {
	...
	const existingDonorIds = existingOffers.map((offer) => offer.donorId);
	...
	const candidates: Candidate[] = donors
		.map((donor) => {
			...
			if (!isEligibleToDonate(profile.lastDonationDate)) {
				return null;
			}
	...
	if (selectedDonors.length === 0) {
		await prisma.bloodRequest.update({
			...
			data: {
				status: BloodRequestStatus.NO_DONOR_FOUND,
			},
		});
	...
	const notificationResults = await Promise.allSettled(
		selectedDonors.map(async (candidate) => {
			try {
				...
				if (!donor) {
					console.log(`Donor not found: ${candidate.donorId}`);

					return;
				}

				console.log(`Preparing notification for donor: ${donor.name}`);
				...
				if (donor.email) {
					...
					const templateData = {
						...
						requiredDate: request.requiredDate.toLocaleDateString("en-BD", {
							timeZone: "Asia/Dhaka",
						}),

						requiredTime: request.requiredTime
							? request.requiredTime.toLocaleTimeString("en-BD", {
									timeZone: "Asia/Dhaka",
									hour: "2-digit",
									minute: "2-digit",
								})
					...
					await transporter.sendMail({
						...
						to: donor.email,
						subject: `🩸 Blood Link — ${request.bloodGroup} Blood Needed`,
						html,
					});

					console.log(`✅ Email sent to ${donor.email}`);
				} else {
					console.log(`⚠️ Donor ${donor.name} has no email`);
				}
				...
				if (donor.phone) {
					...
					console.log(`⚠️ Donor ${donor.name} has no phone number`);
				}
			} catch (error) {
				console.error(
	...
	const donorIds = selectedDonors.map((donor) => donor.donorId);

	try {
		await sendPushNotificationToUsers(donorIds, {
